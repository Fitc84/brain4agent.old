...
→ **GIỚI HẠN (đo được 2026-09-06, xem #33):** `BRN-018` chỉ canh trục `brain_template_version`. Một engine **khác** giữ **nguyên** template `1.4.0` nhưng đổi **thân luật** thì lọt hoàn toàn — bản `1.7.4` lạ đã ghi đè 7 repo đúng như vậy. Nói cho chặt: #12 mới bịt được nửa "engine template cũ"; nửa "engine cùng template, khác thân luật" vẫn hở, và **kỷ luật so hash `deploy:verify` vẫn là thứ duy nhất bắt được nó**.
...
## 32. Thư Mục MẤT `.git` Báo "Sạch" Vì Phép Đo Leo Lên Repo CHA — Ảo Giác Nguy Hiểm Nhất
- **Triệu chứng:** `git -C <repo> status --porcelain` trả **rỗng**. Kết luận tự nhiên: *"repo sạch, di trú được"*. Thực tế thư mục đó **RỖNG HOÀN TOÀN**, không còn cả `.git`; git leo lên thư mục cha và trả trạng thái của **repo khác**.
- **Vì sao nguy hiểm hơn mọi lỗi đo khác:** nó không báo lỗi, không trả mã thoát khác 0, và cho ra đúng cái kết quả mà người đo đang mong muốn (`sạch`). Ba lệnh khác nhau (`status`, `log`, `ls-remote`) đều "chạy được" và đều trả dữ liệu **của repo cha** — đủ tạo một câu chuyện mạch lạc nhưng sai hoàn toàn. Đây là bản nặng hơn của #7 (ở #7 nó chỉ thổi phồng số file bẩn; ở đây nó **giấu hẳn việc một repo đã biến mất**).
- **Phép thử bắt buộc trước MỌI phép đo trên repo vệ tinh:**
