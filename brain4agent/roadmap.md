...
- [ ] **Kế hoạch #05 — Não hóa Nhóm C (DRAFT chờ user duyệt):** Spec-First cho 6 repo chưa có `AGENTS.md`, nguyên tắc "di trú ngữ nghĩa trước — engine sau" chống não song trùng. 6 SPEC theo lớp khó: P01 `block-ads-fb-v2` (A) → P02 `dreamteam4vn` (A+) → P03 `Audit` (B) → P04 `reverse Claude` (B+, governance riêng sống) → P05 `Agent to Product` (C, cộng sinh với Brain OS legacy) → P06 `CausalAgent` (D, GATED chờ user commit đầu tiên). 3 câu hỏi mở chờ user ở `planning/05_2026-08-31_nao-hoa-nhom-c/plan.md` mục 4.
