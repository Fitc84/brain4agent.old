...
- [ ] **Deploy engine hub ra global (P06 #10) — HOÃN theo lệnh user.** Thực hiện đúng `planning/10_2026-09-02_va-bang-marker/specs/OPERATIONS.md` §4. Tới khi deploy, bản global vẫn cũ nên **cấm chạy Bước 0 ở chế độ ghi**; chỉ dùng `--check`.
- [ ] **Rollout khung não v1.4.0 ra ~66 repo theo sóng (P07 #10) — HOÃN theo lệnh user.** Thực hiện đúng `planning/10_2026-09-02_va-bang-marker/specs/OPERATIONS.md` §5; chỉ bắt đầu khi user ra lệnh.
...
- [ ] **`ai-news-radar`** — `state.json` thiếu byte `0x0A` cuối tệp (`BRN-011`). Engine tự sửa được bằng một lần chạy chế độ ghi.
...
- [x] **Vá tất định bằng marker + khung v1.4.0 (v1.7.0, kế hoạch #10, đóng 2026-09-04):** 6 khối `<!-- brain:rule:<id> -->` fail-closed thay toàn bộ regex dò văn xuôi; 2 luật v1.4.0 (Ký ức Lạnh, Structural Extension) tốn đúng 9 dòng engine; BRN-016/017; test **193 → 241**; lõi vá **165 → 123** dòng (−25%); engine **1447 → 1432** dòng. *Phạm vi đã đóng: engine + hub + lưới test + CI. P06 deploy global và P07 rollout fleet hoãn theo lệnh user.*
