...
4. `npm run deploy:verify`: trước khi deploy, exit **2** là ĐÚNG (global cố ý đi sau hub); sau khi deploy phải về **0**. **KHÔNG ghim số version vào cổng này** — đối chiếu bằng hash, không bằng chữ.
...
| # | Việc | Kiểm | Ai |
| :-- | :--- | :--- | :-- |
| D1 | ✅ **ĐÃ LÀM ở #10**: bump 3 trục (`ENGINE_VERSION`/`package.json` → `1.7.0`, `BRAIN_TEMPLATE_VERSION` → `1.4.0`), chạy engine chế độ ghi trên hub. *Các bản vá PATCH sau #10 đã nâng trục dự án tiếp — xem `changelog.md`; trục KHUNG NÃO vẫn `1.4.0`.* | `version-sync` xanh; `--check .` = 0 | agent |
...
| D4 | **Xin phép user bằng lời**: "sắp ghi bản engine hiện hành của hub / khung 1.4.0 ra global — mọi repo chạy Bước 0 sau đó sẽ thấy `CẦN NÂNG CẤP`" (đọc số thật bằng `node .agents/skills/.xay-dung-nao-bo/scripts/init_brain.js --version` của hub) | | **user** |
| D5 | `npm run deploy` ⇒ exit 0, `diff=0 missing=0 cmd=ok` | | agent |
| D6 | Kiểm tay: hash từng file; `node <global>/init_brain.js --version` **khớp ĐÚNG output của** `node .agents/skills/.xay-dung-nao-bo/scripts/init_brain.js --version` của hub (trục khung não phải là `template 1.4.0`); `node <global>/init_brain.js --check <hub>` = 0 | | agent |
...
Sau D5, **mọi repo vệ tinh** chạy Bước 0 (bản cũ, chế độ ghi) sẽ được engine MỚI vá ngay — đây chính là lý do rollout phải diễn ra sớm sau deploy và có kiểm soát (§5), và là lý do D4 phải nói rõ với user.
...
Nếu user **hoãn** WP7: ghi vào nhật ký `plan.md` "rollout hoãn theo lệnh user, engine đã deploy", cột `fleet` của Exit Gates để ⏸ — kế hoạch được đóng ở mức "engine + hub + global" (giống cách #09 để lại H6).
