...
2. **BẮT BUỘC DẠNG SPEC PACKAGE — CẤM PLAN PHẲNG/MỎNG:** mỗi kế hoạch phải tách thành bộ SPEC nhiều file đặt trong `specs/`, không dồn hết vào một `plan.md`.
...
2. Khi viết regex thay thế, các chuỗi $` và $& và $' và $$ phải được giữ NGUYÊN VĂN.

### H. Quy tắc Giám Sát Tác Vụ Ngầm
1. Bảng mẫu thay thế: tiền tố $` , toàn khớp $& , hậu tố $' , dấu đô-la $$ .
2. CẤM polling file log liên tục theo giây.
